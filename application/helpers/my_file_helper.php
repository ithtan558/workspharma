...

{
 ...
 function SaveImage($im, $filename,$image_type,$save_to_file,$quality) {
  ...
  if (!function_exists('is_attachment_exists')){
      function is_attachment_exists($filename){
          $CI = &get_instance();
          $filePath = ROOT_PATH . '/files/project_attachment/' . $filename;
          //echo $filePath;
          //exit;
  ...
  if (!function_exists('is_file_exists')){
      function is_file_exists($filename,$folder = 'logo'){
          $CI = &get_instance();
          $filePath = ROOT_PATH. '/public/images/'.$folder.'/' . $filename;
         //echo $filePath;
          if((is_file($filePath))&&(file_exists($filePath))){
              return true;
          }
  ...
  if (!function_exists('is_file_exists1')){
      function is_file_exists1($filename){
          $CI = &get_instance();
...
if (!function_exists('encrypt_name')){
    function encrypt_name($filename){
        $arrayFilename=explode('.', $filename);
        $n=count($arrayFilename);
        $name='';
        foreach ($arrayFilename as $key => $value) {
            if($key<$n-1){
                $name.=$value;
            }
            else{
