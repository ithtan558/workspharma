...
class Job extends Application
{
    ...
    public function __construct(){
        ...
        //load all skills
        $condition_categories=array('categories.status_temp1'=>0);
        $order=array('view_search','DESC');
        $getCategories = $this->skills_model->getCategories($condition_categories,NULL,NULL,NULL,$order);
        ...
        $conditions=array('categories.is_active'=>1,'categories.status_temp1'=>0);
        $order=array('categories.view_search','DESC');
        $limit=array(6);
        $getCategoriesPopular = $this->skills_model->getCategories($conditions,NULL,NULL,$limit,$order);
        ...
        $this->lang->load('enduser/formValidation',$this->my_config->item('language_code'));
        ...
        //{
            ...
            foreach($html->find('.first-group .job .title a') as $elementItems)
            {
                ...
                if($getJobs->num_rows()>0);
                    ...
                    //listCategories
                    $stringSkills='';
                    $listCategories=$this->skills_model->getCategory();
                    ...
                    $dataJob['views']=1;
                    $this->jobs_model->addJob($dataJob);
                    //check emplpyer elxit
                    if($getUsers->num_rows()>0){
                        ...
                        foreach ($items->find('#jd-photo-slider .item') as $itemPhoto){
                            ...
                            $this->jobs_model->addEmployerFiles($dataUserEmployersFiles);
                        ...
                        $idUserEmployer=$this->jobs_model->addEmployer($dataUserEmployer);
                    }
                }
                //die;
            //}
                    ...
                    foreach ($items->find('.job-tag-add-skill1 span.addable') as $tag) {
                        ...
                        foreach ($listCategories->result() as $item) {
                            if(preg_match("/".$item->category_name."/",$nameSkill)){
                                ...
                                $idSkill=$this->skills_model->addCategory($insertDataSkill);
                                break;
                            }
                        }
                        if(!isset($idSkill)){
