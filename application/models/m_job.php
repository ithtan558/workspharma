	 ...
	 */
	 ...
	 function getJobs($conditions=array(),$fields='',$like=array(),$limit=array(),$conditionOrCategories = '',$conditionOrCities = '',$orderby = array(), $or_like = null, $stringEmployers = '',$where_not_in=array())
	 {
		...
		if(is_array($where_not_in) and count($where_not_in)>0)
		 ...
		 $this->db->from('jobs');
		 $this->db->join('categories', 'categories.id = jobs.category_ids', 'left');
		 ...
		 if($fields!='')
			 ...
			 $this->db->select('jobs.hide_infomation,jobs.date_created as jdate_created, categories.category_name,jobs.date_created as j_date_created,jobs.type_contact,jobs.email_contact,jobs.name_contact,jobs.address_contact,jobs.people_contact,
			 	jobs.phone_contact,jobs.language_contact,jobs.experience_skill,jobs.date_expiration,
			 	jobs.education_id,jobs.type_id,jobs.level_id,jobs.fromage, jobs.toage,jobs.gender,jobs.salary,jobs.experience_skill,
			 	jobs.favourites,jobs.user_id,jobs.status,jobs.user_id,jobs.id,jobs.title,jobs.alias,jobs.gender,
			 	jobs.fromage, jobs.toage,jobs.year_exp, jobs.qty, jobs.salary_min,jobs.salary_max,
			 	jobs.description, jobs.date_created as j_date_created, jobs.category_ids,jobs.city_ids,jobs.category_ids,jobs.views,
			 	job_apply.date_created, users.user_name, job_apply.status as jastatus,
			 	user_employers.company, user_employers.address,user_employers.contact, user_employers.phone,user_employers.num_of_staff, user_employers.website');
