...
require_once(APPPATH . 'models/admin/m_application_admin.php');
	 class M_employer extends M_application_admin {
	/**
	 * Constructor
	 *
	 */
	 ...
	 */
	 function getEmployer($conditions=array(),$fields='',$like=array(),$limit=array(),$orderby = array(), $params = null,$conditionOrCategories='',$or_like=array())
	 {
		...
		if($fields!='')
	 		...
	 		$this->db->select('users.*, users.id as uid, user_employers.*');
		$result = $this->db->get();
		return $result;
		$result = $this->db->get();
		return $result;
	 ...
	 * @return	void
	 */
	 function deleteEmployer($id=0,$conditions=array())
	 {
